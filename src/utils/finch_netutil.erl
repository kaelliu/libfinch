%%% File : finch_netutil.erl
%%% Description : server's help util code 
%%% Author : kael liu
%%% Date : 2012-07-31 Tuesday

-module(finch_netutil).
-export([tcp_name/3,getaddr/2,check_tcp_listener_address/2]).
-export([ntoa/1,ntoab/1]).
-export([recv/1]).
-export([throw_on_error/2,sock_operation/2]).

%% do function on sock
sock_operation(Sock,Fun)->
	case Fun(Sock) of
		{ok,Result} 	-> Result;
		{error,Reason} -> 
...
% formate a module name
tcp_name(Prefix,IpAddress,Port) when is_atom(Prefix) andalso is_number(Port)->
	list_to_atom(
     lists:flatten(
...
%% inet_parse:address takes care of ip string, like "0.0.0.0" of a host
...
%% resolve ip family,ipv4 or ipv6
resolve_family({_,_,_,_},auto)->inet;
resolve_family({_,_,_,_,_,_},auto)->inet6;
